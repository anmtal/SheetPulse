...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Helper: Simulate Logging In (Mock Mode)
    function simulateLogin(email) {
        ...
        // Auto-redirect to Stripe Payment Link if triggered via Gatekeeper
        if (sessionStorage.getItem('triggerPaymentAfterLogin') === 'true') {
            sessionStorage.removeItem('triggerPaymentAfterLogin');
            window.location.href = 'https://buy.stripe.com/aFaaEXdwx21Q57B7dI2cg00';
        }
    }
    ...
    if (supabaseClient) {
        supabaseClient.auth.onAuthStateChange((event, session) => {
            if (session && session.user) {
                // User logged in live
                simulateLogin(session.user.email);
            } else {
